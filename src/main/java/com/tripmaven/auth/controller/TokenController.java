...
public class TokenController {
    ...
    private final MembersService membersService; // 사용자 정보를 처리하는 서비스
    private final TokenService tokenService; // 리프레시 토큰 관련 서비스
    ...
    public ResponseEntity<?> reissue(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            if (jwtUtil.isTokenExpired(token)) {
                ...
                tokenService.deleteByRefresh(token);
        ...
        // 사용자 정보 조회
        MembersEntity membersEntity = membersService.searchByMemberEmail(username).toEntity();
        if (membersEntity == null) {
            return new ResponseEntity<>("user not found", HttpStatus.NOT_FOUND);
        }

        // 리프레시 토큰이 유효한지 확인합니다.
        Optional<TokenEntity> tokenOptional = tokenService.findByTokenValue(token);
        if (tokenOptional.isEmpty() || !tokenOptional.get().getMembers().equals(membersEntity)) {
        ...
        if (!tokenEntity.getStatus().equals("activated")) {
